...
gen_vulnerability_factors_pathways_sonder <- function(NN=NULL){
  ...
  NN$dist.natal.2plus <- ifelse(NN$dist.natal <=2, "=< 2", "> 2")
  ...
  NN$freq.net.cat1 <- ifelse(NN$freq.net >= 4, 1, 0)
  ...
  NN$freq.news.cat1 <- ifelse(NN$freq.news >= 4, 1, 0)
  ...
  NN$family.cat1 <- ifelse(NN$family > 4, 1, 0)
  ...
  NN <- NN %>%
    mutate(comm.index.2plus = ifelse(comm.index >= 2, 1, 0))
