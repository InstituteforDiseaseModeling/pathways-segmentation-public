...
################################################################################
# GENERATE OUTCOME VARIABLES | PATHWAYS | SONDER
################################################################################
...
gen_outcome_variables_pathways_sonder <- function(NN){
  ...
  NN <- NN %>%
    mutate(
      hb.son = ifelse(
        rowSums(!is.na(dplyr::select(., starts_with("hb.son")))) == 0,
        NA,  # set to NA if all `hb.sonX` are NA
        rowSums(dplyr::select(., starts_with("hb.son")), na.rm = TRUE)))
  ...
  NN <- NN %>%
    mutate(
      hb.dau = ifelse(
        rowSums(!is.na(dplyr::select(., starts_with("hb.dau")))) == 0,
        NA,
        rowSums(dplyr::select(., starts_with("hb.dau")), na.rm = TRUE)))
  ...
  IO31 <- NN %>% dplyr::select(starts_with("IO31"))
  anthro <- NN %>% dplyr::select(caseid, starts_with("IO31_7"), starts_with("IO31_8"), starts_with("T_S03a_7"), starts_with("T_S03a_8"))
  ...
  son6 <- anthro %>% dplyr::select(caseid, IO31_7_6, age.son6) %>% dplyr::filter(!is.na(IO31_7_6) & !is.na(age.son6))
  son7 <- anthro %>% dplyr::select(caseid, IO31_7_7, age.son7) %>% dplyr::filter(!is.na(IO31_7_7) & !is.na(age.son7))
  son8 <- anthro %>% dplyr::select(caseid, IO31_7_8, age.son8) %>% dplyr::filter(!is.na(IO31_7_8) & !is.na(age.son8))
  son9 <- anthro %>% dplyr::select(caseid, IO31_7_9, age.son9) %>% dplyr::filter(!is.na(IO31_7_9) & !is.na(age.son9))
  son10 <- anthro %>% dplyr::select(caseid, IO31_7_10, age.son10) %>% dplyr::filter(!is.na(IO31_7_10) & !is.na(age.son10))
  ...
  dau6 <- anthro %>% dplyr::select(caseid, IO31_8_6, age.dau6) %>% dplyr::filter(!is.na(IO31_8_6) & !is.na(age.dau6))
  dau7 <- anthro %>% dplyr::select(caseid, IO31_8_7, age.dau7) %>% dplyr::filter(!is.na(IO31_8_7) & !is.na(age.dau7))
  dau8 <- anthro %>% dplyr::select(caseid, IO31_8_8, age.dau8) %>% dplyr::filter(!is.na(IO31_8_8) & !is.na(age.dau8))
  dau9 <- anthro %>% dplyr::select(caseid, IO31_8_9, age.dau9) %>% dplyr::filter(!is.na(IO31_8_9) & !is.na(age.dau9))
  dau10 <- anthro %>% dplyr::select(caseid, IO31_8_10, age.dau10) %>% dplyr::filter(!is.na(IO31_8_10) & !is.na(age.dau10))
  ...
  # SUBSET
  NN <- NN %>%
    dplyr::select(caseid, survey, breastfed, breastfed.6mo, dau.hosp_sum, kid.hosp, kid.ill.3mo, kid.ill.3plus, kid.ill.sum, kid.stunt, son.hosp_sum,
                  stunt_sum, death.u5, death.u5.dau, death.u5.son, anc.1sttri, anc.4plus, anc.start, anc.total, hb.dau, hb.dau.yn,
                  hb.son, hb.son.yn, hb.sum, hb.sum.3plus, hb.sum.yn, preg.current, preg.live, preg.loss, preg.loss.num, preg.num,
                  preg.risk, preg.risk.anc, preg.risk.deliver, preg.risk.diff, preg.risk.surgery, depress, depress.mean, fp.current.any,
                  fp.current.modern, fp.previous.any, fp.previous.modern, her.ill.3mo, her.ill.4plus, her.ill.sum, her.wasting)
