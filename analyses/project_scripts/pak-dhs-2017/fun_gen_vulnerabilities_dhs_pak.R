...
gen_vulnerability_factors_dhs_pak <- function(IR=NULL, BR=NULL, HH=NULL, MR=NULL, DHS=8){
  ...
  HH <- HH %>%
    dplyr::mutate(hh.cook.inside.yn = case_when(hv241 == "in the house" ~ "Yes",
                                                !is.na(hv241) ~ "No"))


  # CLEAN COOKING FUEL
  HH <- HH %>%
    dplyr::mutate(hh.clean.fuel = case_when(hv226 %in% c("electricity", "lpg", "natural gas", "biogas") ~ "Yes",
                                            !is.na(hv226) ~ "No"))
  ...
  HH <- HH %>%
    dplyr::mutate(latrine = case_when(hv205 %in% c("ventilated improved pit latrine (vip)",
                                                   "pit latrine without slab/ open pit",
                                                   "pit latrine without slab/open pit",
                                                   "bucket toilet",
                                                   "hanging toilet/latrine", "other") ~ 1,
                                      !is.na(hv205) ~ 0))
  ...
  ###################################
  # Urban slum - UN definition - INFORMAL HOUSING
  ...
  IR <- IR %>%
    dplyr::mutate(dv.nofamily.contact = case_when(
      d101d %in% c("yes", "often", "sometimes", "yes, but not in the last 12 months", "yes, but frequency in last 12 months missing") ~ "Yes",
      d101d %in% c("no", "never") ~ "No",
  ...
  # HUSBAND/PARTNER'S EDUCATION LEVEL - CAT
  IR <- IR %>%
    dplyr::mutate(partner.ed.level.cat = case_when(v701 %in% c("primary", "others", "higher", "secondary") ~ "education",
