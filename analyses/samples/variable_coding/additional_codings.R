...
IR.seg <- IR %>% dplyr::select(c("v001","v002","v005","v021","v023", "v024", "v201", "wt", "caseid", "ed.lev", "age.yrs":"med.index.3plus"))

IR.seg <- IR.seg %>% dplyr::select(-c("dv.nacnt", "dv.sum", "child.pref.discrep",
                               "no.partner", "jd.nacnt", "wd.nacnt", "wd.sum", "jd.sum", "med.sum"))

HH.seg <- HH %>% dplyr::select(c("v001", "v002", "num.15up", "num.15up.cat", "num.15up.4plus", "num.under15", "num.under15.cat", "highestyearsedinHH.yrs",
...
BR.seg <- BR %>% dplyr::select(c("caseid", "kids.under5":"num.kids.away.cat"))

BR.seg <- BR.seg %>% dplyr::select(-c("lives.away"))
...
vulnerability_eth <- base::merge(IR.seg, HH.seg, by=c("v001", "v002"), all.x=T)
vulnerability_eth <- base::merge(vulnerability_eth, SWPR, by="caseid", all.x=T)
vulnerability_eth <- base::merge(vulnerability_eth, BR.seg, by="caseid", all.x=T)
